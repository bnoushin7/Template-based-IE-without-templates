...
def selectional_preferencer(indata, out_dict):
    ...
    uniques_list = list(
        set(vals for values in my_dict.values() for vals in values))  # #list of all unique tupples (name and NER)

    indexing_dict = {val: idx for idx, val in enumerate(
        uniques_list)}  # I have used this dictionary to know the index of each syntactic relations in the list to be
    ...
    other_list = [first for first, second in uniques_list if
                  second == "OTHER"]  # The tree above lists have only 1 element when printing,
...
sp_out = selectional_preferencer("/home/noushin/summerTask/dev-muc3-0001-0100.conll_bk", "/home/noushin/summerTask/select_pe")
